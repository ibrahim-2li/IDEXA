        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.expertise.update', $expertiseArea) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700">Card Title</label>
                            <input type="text" name="title" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ old('title', $expertiseArea->title) }}" required>
                            @error('title') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>

                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700">Primary Description</label>
                            <textarea name="description_1" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('description_1', $expertiseArea->description_1) }}</textarea>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="description_2" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description_2', $expertiseArea->description_2) }}</textarea>
                            @error('description_2') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                        
                        <div class="mb-6">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Display Image Override</label>
                            @if($expertiseArea->image)
                                <div class="mb-4">
                                    <img src="{{ $expertiseArea->image }}" alt="Preview" class="h-24 object-contain rounded shadow-sm border border-gray-200">
                                </div>
                            @endif
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="link" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ old('link', $expertiseArea->link) }}" required>
